...
> **Note on Automation Rate**: The automation rate with the active risk policy is controlled because the system deliberately abstains on uncertain or high-exposure matches rather than auto-resolving everything.
...
6. **Formally Calibrated ML Probabilities**: The ML matcher utilizes raw Logistic Regression `predict_proba()` estimates—validated by precision, recall, and Brier score on a modest held-out split—rather than a formally calibrated probability via Platt scaling or isotonic regression.
